...
- جولة `AWC-RC-057` (2026-04-19) أغلقت شريحة `CSP Report-Only` في Phase 3 على admin web: إضافة security headers عالمية عبر `next.config.mjs` تشمل `Content-Security-Policy-Report-Only` و`Permissions-Policy` مع `Referrer-Policy` و`X-Content-Type-Options` و`X-Frame-Options`، مع دعم اختياري لـ `WAIN_ADMIN_CSP_REPORT_URI` وإدراج `upgrade-insecure-requests` فقط في production؛ تم تثبيت السلوك عبر regression test جديد `lib/security/next-config-security-headers.test.ts`.
- جولة `AWC-UI-045` (2026-04-19) بدأت Phase 4 بإغلاق `UX-3` (central date formatting): إضافة helper مركزي `formatArabicDate` داخل `lib/finance/read-model-formatters.ts` مع locale/timezone ثابتين (`ar-PS-u-nu-latn`, `UTC`) وخيار `dateOnly`، وربط `formatAdminDate` به، ثم إزالة الاستدعاءات المباشرة `toLocaleDateString()` من واجهتي العروض والقصص (`components/content/offers-management-shell.tsx` و`components/content/stories-management-shell.tsx`)؛ التحقق مر عبر Vitest مركز (`lib/finance/read-model-formatters.test.ts` + `components/content/content-management-shells.test.tsx`)، ثم `npm run test:security` (`44/44`) ثم `npx tsc --noEmit --pretty false`.
- جولة `AWC-UI-046` (2026-04-19) أغلقت `UX-2` في `venue-create-dialog`: استبدال إدخال ساعات العمل الخام بصيغة JSON بواجهة يومية منظّمة (مفتوح/من/إلى/يمتد لليوم التالي) مع بقاء نفس عقد payload، تعريب حقول التواصل (`Instagram/WhatsApp/Facebook/Website`) إلى صياغة عربية أولًا، وإضافة قائمة أجواء جاهزة قابلة للاختيار مع دمجها في `tags.mood`؛ التحقق مر عبر Vitest مركز (`components/venues/venue-create-dialog.test.tsx`)، ثم `npx tsc --noEmit`، ثم `npm run test:security` (`44/44`) و`npm run build`.
- جولة `AWC-UI-047` (2026-04-19) أغلقت `UX-1` لصفحة `/admin/config` عبر إعادة تنظيم الواجهة حسب المهام بدل البنية: إضافة خريطة عمل واضحة بثلاث خطوات (`تحديث الأسعار وحفظ المسودة` → `المراجعة والنشر` → `الاسترجاع عند الحاجة`)، توزيع إجراءات الأوامر على نفس الخطوات داخل shell مع الإبقاء على نفس command contracts وRBAC والسلوك التشغيلي، وتوسيع baseline test لتثبيت الهيكل task-driven؛ التحقق مر عبر Vitest مركز (`components/config/config-governance-shell.test.tsx` + `components/config/config-governance-route.test.tsx`)، ثم `npx tsc --noEmit`، ثم `npm run test:security` (`44/44`) و`npm run build`.
- جولة `AWC-UI-048` (2026-04-19) أغلقت دليل Visual QA لـ Gate D بعد إكمال UX Closure: تشغيل `npm run capture:ui-a02` أعاد نتيجة نظيفة (`15/15` screenshots، `0` redirects إلى sign-in، `0` failures) وتحديث artifacts (`admin_web_console_ui_a02_baseline_capture.json` + `.md` + مجلد `ui_a02_baseline_screenshots/`)، ثم تشغيل `npm run capture:ui-a02:triage` أعاد `ok=true` بدون critical visual findings (`changedDimensions=0`, `missingCurrent=0`) مع تحديث تقارير `admin_web_console_ui_visual_diff_triage_ui_026.json` و`.md`.
