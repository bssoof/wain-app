...
| **UX-3** | `toLocaleDateString()` بلا locale ثابت | Closed in `AWC-UI-045` |
...
- **Progress Note (2026-04-19 / AWC-RC-055)**: تم إغلاق انحراف رسالة finance في وضع `unavailable` بحيث لا تُضاف بادئة `Unavailable:` على الرسائل العامة، وبالأخص رسالة `FIXTURE_FALLBACK_DISABLED_MESSAGE_AR` عند حظر fixture fallback في production؛ كما تمت إضافة regression test ضمن `fixture-fallback-policy.security.test.ts` وتحقق كامل عبر `test:security` (`44/44`) للحفاظ على اتساق الرسائل العربية عبر بوابة الأمن.
...
- **Progress Note (2026-04-19 / AWC-RC-056)**: تم تفعيل structured denial logging على طبقة admin web عبر emitter موحد `[SECURITY_AUDIT]` مع JSON واضح في نقطتي الاختناق (`route-guards` + `/api/admin/command/*` proxies) وتغطية اختبارات regression (`admin-security-audit.test.ts` و`route-guards.security.test.ts`) مع نجاح `test:security` و`tsc`; المتبقي لإغلاق البند بالكامل: إثبات visibility في dashboard التشغيلي وربط retention policy لمدة 90 يومًا على sink المعتمد.

### OPS-4 — CSP Report-Only + Permissions-Policy

- **Owner**: Frontend/App
- **المدة**: 0.5 يوم
- **Exit Evidence**: headers ظاهرة في response على كل مسارات admin + regression test يمنع الانحراف.
- **Progress Note (2026-04-19 / AWC-RC-057)**: تم تفعيل security headers عبر `next.config.mjs` على `/:path*` وتشمل `Content-Security-Policy-Report-Only` و`Permissions-Policy` مع hardening مكمل (`Referrer-Policy`, `X-Content-Type-Options`, `X-Frame-Options`)، مع دعم `WAIN_ADMIN_CSP_REPORT_URI` وإضافة `upgrade-insecure-requests` فقط في production؛ تمت إضافة regression test (`lib/security/next-config-security-headers.test.ts`) لتثبيت السلوك.
...
- **Progress Note (2026-04-19 / AWC-UI-045)**: تم تنفيذ `UX-3` عبر helper مركزي `formatArabicDate` (locale/timezone ثابتان) في `lib/finance/read-model-formatters.ts` وربطه مع `formatAdminDate`، مع استبدال كل استخدامات `toLocaleDateString()` المتبقية في صفحات المحتوى (`offers/stories`)؛ التحقق مر عبر Vitest مركز + `test:security` + `tsc`.
- **Progress Note (2026-04-19 / AWC-UI-046)**: تم تنفيذ `UX-2` في `venue-create-dialog` عبر إزالة إدخال JSON الخام لساعات العمل واستبداله بواجهة عربية يومية منظّمة (مفتوح/من/إلى/يمتد لليوم التالي)، وتعريب حقول التواصل (`Instagram/WhatsApp/Facebook/Website`) إلى صياغة عربية، وإضافة قائمة أجواء جاهزة تُدمج مع الوسوم الإضافية في `tags.mood` بدون تغيير عقود backend؛ التحقق مر عبر Vitest مركز + `tsc` + `test:security` + `next build`.
- **Progress Note (2026-04-19 / AWC-UI-047)**: تم تنفيذ `UX-1` في `/admin/config` عبر تحويل الصفحة إلى تنظيم task-driven بثلاث مراحل تشغيلية واضحة (حفظ المسودة، المراجعة/النشر، الاسترجاع) مع توزيع أزرار التنفيذ على كل خطوة بدل كتلة أوامر واحدة، وإضافة خريطة عمل للمشغّل؛ تم الحفاظ على العقود والصلاحيات دون تغيير، والتحقق مر عبر Vitest مركز + `tsc` + `test:security` + `next build`.
- **Progress Note (2026-04-19 / AWC-UI-048)**: تم إغلاق Visual QA الخاص بـ Gate D: تشغيل `capture:ui-a02` أعاد `15/15` screenshots و`0` failures و`0` redirects إلى sign-in، ثم `capture:ui-a02:triage` أعاد `ok=true` بدون critical visual findings، مع تحديث artifacts في `docs/release/admin_web_console_ui_a02_baseline_capture.json` و`docs/release/admin_web_console_ui_visual_diff_triage_ui_026.json` وما يقابلها من ملفات `.md` ومجلد screenshots.
...
- [x]  CSP Report-Only + Permissions-Policy headers
...
- [x]  `/admin/config` منظمة حسب المهام
- [x]  venue create Arabic-first بلا JSON خام
- [x]  `formatArabicDate` مركزي في كل الصفحات الحساسة
- [x]  Visual QA pass vs Vercel Web Interface Guidelines
