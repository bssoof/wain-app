...
- **المدة**: 0.25–0.5 يوم
- **Exit Evidence**:
    - [ ]  قرار معماري موثّق: re-export رسمي أم تعديل حدود الاختبار
    - [ ]  emulator security callable path أخضر
    - [ ]  لا compatibility hacks مخفية
...
- **المدة**: 0.5 يوم
- **Exit Evidence**:
    - [ ]  السكربت يقرأ contract فعلي بعد `npm run build` من `lib/wallet_runtime_maintenance.js` أو metadata مخصص للـ verification، وليس source text من `index.ts`
    - [ ]  اختبار يمر على البنية الحالية بعد التفكيك
    - [ ]  false-fail path مُزال
...
- **Exit Evidence**:
    - [ ]  checklist status مشتق من CI/run evidence تلقائيًا
    - [ ]  لا stale pass-count snapshot (حذف 219/219)
    - [ ]  حالة صريحة: `NO-GO` / `CONDITIONAL GO` / `GO`
...
- [ ]  contract ambiguity محلولة ومُوثّقة
- [ ]  verify script يعكس البنية الحالية
- [ ]  checklist مشتقة تلقائيًا من CI artifact
- [ ]  `npm run test:emulator:aggregate` = 190/190 على بورت نظيف
