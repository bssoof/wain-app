...
24. إذا كان العرض من نوع نسبة (`percent`)، أدخل إجمالي الفاتورة قبل الخصم وتأكد من ظهور:
   - قبل الخصم
   - التوفير المؤكد
   - بعد الخصم
25. بعد صرف عرض نسبة مع إدخال الفاتورة، افتح `User Stats` وتأكد أن:
   - `العروض المستخدمة` ارتفعت
   - `التوفير المؤكد` ارتفع أيضًا
26. جرّب صرف عرض نسبة بدون إدخال الفاتورة وتأكد أن:
   - الصرف ينجح
   - `العروض المستخدمة` ترتفع
   - `التوفير المؤكد` لا يرتفع لهذا العرض
27. جرّب redeem فاشل أو token غير صالح وتأكد أن الخطأ يظهر بوضوح وأن التدفق لا ينكسر.
28. تأكد أن كل `must work now` flows تنتهي بـ success/error feedback واضح وقابل للتعافي.
