  ...
  await tester.pumpWidget(
    ProviderScope(
      overrides: [
        ...
        // The route sweep validates demo surfaces, not Riverpod's stream
        // lifecycle. Pin auth to a settled signed-out value so disposing each
        // short-lived router cannot race a still-loading StreamProvider.
        ...
        for (final textScale in const <double>[1.0, 1.3]) {
          for (final tagCase in const <({String label, int count})>[
            ...
            testWidgets('$name lays out without overflow', (tester) async {
              ...
              expect(
                badge.top,
                greaterThanOrEqualTo(panel.bottom - 0.5),
                reason: name,
              );
